<template>
      ...
      <div class="details-header">
        ...
        <div class="details-navigation">
          ...
          <Calendar
            ...
            @update:modelValue="updateSelectedDateCalendar"
            dateFormat="yy/mm/dd"
            :showIcon="true"
            iconDisplay="input"
            placeholder="日付選択"
  ...
  export default {
    ...
    methods: {
      formatSelectedDateDisplay(date) {
        if (!date) return "";
      ...
      updateSelectedDateCalendar(value) {
        this.$emit('update:selectedDateCalendar', value);
      },
      onGanttDateSelect(event) {
        this.$emit('gantt-date-select', event);
      },
      openGanttShiftEditor(date, staff, event) {
        this.$emit('open-gantt-shift-editor', date, staff, event);
      },
      ...
      // 以下のメソッドは親コンポーネントから受け取る必要があります
      hasHourRequirements(date, hour) {
        return false;
      },
      getHourRequirements(date, hour) {
        return [];
      },
      hasRequirementShortage(date, req) {
        return false;
      },
      getAssignedStaffCount(date, req) {
        return 0;
      },
      hasStaffWarningsAllStores(staffId) {
        return false;
      },
      getStaffWarningsAllStores(staffId) {
        return [];
      },
      isHoursOutOfRangeAllStores(staffId) {
        return false;
      },
      isStoreClosedOnDate(date) {
        return false;
      },
      calculateTotalHours(staffId) {
        return 0;
      },
      getOtherStoreHoursBreakdown(staffId) {
        return [];
      },
      calculateTotalHoursAllStores(staffId) {
        return 0;
      },
      getShiftForStaff(date, staffId) {
        return null;
      },
      hasShiftViolation(date, staffId) {
        return false;
      },
      canStaffWorkOnDate(staff, date) {
        return true;
      }
    }
  };
  ...
  .gantt-staff-info.hours-violation {

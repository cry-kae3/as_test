<template>
  ...
  export default {
    ...
    methods: {
      ...
      confirmQuickDelete(shift) {
        this.$emit('quick-delete-shift', shift);
      },
      // 各種計算やチェック関数は親コンポーネントから受け取る
      formatHours(hours) {
      ...
      calculateTotalHours(staffId) {
        // 親コンポーネントのメソッドを参照する場合は、propsとして渡すか、
        // この関数自体を親から受け取る必要があります
        return 0;
      },
      ...
      hasDateWarnings(date) {
        return false;
      },
      getDateWarnings(date) {
        return [];
      },
      ...
      getShiftViolations(date, staffId) {
        return [];
      },
      canStaffWorkOnDate(staff, date) {
        return true;
      },
      getWorkAvailabilityTooltip(staff, date) {
        return '';
      },
      getWorkUnavailabilityReason(staff, date) {
        return '';
      }
    }
  };
  ...
  .staff-row.hours-violation {
    background-color: #fee2e2 !important;
  }
  
  .staff-info.hours-violation {
    background-color: #fee2e2 !important;
  }

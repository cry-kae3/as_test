<template>
  ...
  export default {
    ...
    methods: {
      getDailyShiftStaff(date) {
        if (!date || !this.staffList) return [];
  
        const dayShifts = this.shifts.find((shift) => shift.date === date);
      ...
      getDailyRequirements(date) {
        return [];
      },
      hasStaffingShortage(date, req) {
        return false;
      },
  ...
  .staff-summary-card.hours-violation {
    background: #fee2e2;
    border-color: #ef4444;
  }

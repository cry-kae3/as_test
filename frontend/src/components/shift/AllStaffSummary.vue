    ...
    <div class="all-staff-shift-summary">
      ...
      <div class="all-staff-table-container">
        <table class="all-staff-table">
          ...
          <tbody>
            <tr
              v-for="staff in filteredAndSortedStaff"
              :key="`all-summary-${staff.id}`"
              class="all-staff-row"
              :class="{
                'has-warnings': hasStaffWarnings(staff.id),
                'hours-violation': isHoursOutOfRange(staff.id),
              }"
            >
              <!-- スタッフ名 -->
              ...
              <td class="store-hours-cell">
                <div class="store-hours-list">
                  <div
                    v-for="breakdown in getStoreHoursBreakdown(staff.id)"
                  ...
                  <div
                    v-if="getStoreHoursBreakdown(staff.id).length === 0"
                    class="no-hours"
                  >
              ...
              <td class="status-cell">
                <div class="status-indicators">
                  <div
                    :class="[
                      'status-indicator',
                      getStatusInfo(staff.id).class,
                    ]"
                    :title="getStatusInfo(staff.id).title"
                  >
                    <i :class="getStatusInfo(staff.id).icon"></i>
                    <span class="status-text">{{
                      getStatusInfo(staff.id).text
                    }}</span>
                  </div>
                </div>
              </td>
            ...
            <!-- データがない場合の表示 -->
            <tr
              v-if="filteredAndSortedStaff.length === 0"
              class="no-data-row"
            >
  ...
  export default {
    ...
    setup(props) {
      ...
      const filteredAndSortedStaff = computed(() => {
        ...
        // 店舗検索
        if (filters.searchStore.trim()) {
          const searchTerm = filters.searchStore.toLowerCase();
          filtered = filtered.filter((staff) => {
            const storeNames = getStoreHoursBreakdown(staff.id).map(
        ...
        filtered.sort((a, b) => {
          ...
          switch (filters.sortBy) {
            case "name":
              ...
              break;
            case "totalHours":
              aValue = calculateTotalHours(a.id);
              bValue = calculateTotalHours(b.id);
              break;
            case "storeCount":
              aValue = getStoreHoursBreakdown(a.id).length;
              bValue = getStoreHoursBreakdown(b.id).length;
              break;
            case "status":
              aValue = getStatusPriority(a.id);
              bValue = getStatusPriority(b.id);
              break;
            default:
              return 0;
          }
      ...
      // プレースホルダー関数（親コンポーネントで実装が必要）
      const getStoreHoursBreakdown = (staffId) => {
        return [];
      };
      ...
      const getStatus = (staffId) => {
        ...
        const staff = props.allStaff.find((s) => s.id === staffId);
        ...
        const hasOtherWarnings = checkOtherWarnings(staffId);
        if (hasOtherWarnings) {
          return "warning";
        }
      ...
      const checkOtherWarnings = (staffId) => {
        return false;
      };
  
      const getStatusPriority = (staffId) => {
        const status = getStatus(staffId);
      ...
      const resetFilters = () => {
        ...
        nextTick(() => {
          console.log("フィルターがリセットされました:", filters);
        });
      };

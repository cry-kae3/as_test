...
const actions = {
    ...
    async fetchStaffTotalHoursAllStores({ commit }, { year, month, storeId }) {
        ...
        try {
            const response = await api.get('/shifts/staff-total-hours', {
        ...
        try {
            ...
            if (response.data.summary && response.data.summary.staffingWarnings) {
                commit('setValidationResults', {
                    ...
                    warnings: response.data.summary.staffingWarnings
                });
            }
...
const mutations = {
    ...
    updateShiftStatus(state, { year, month, storeId, status }) {
        if (state.currentShift &&
            ...
            state.currentShift.store_id === parseInt(storeId)) {
    ...
    addShiftAssignment(state, { date, assignment }) {
        ...
        if (staffSummary) {
            ...
            state.currentShiftData.summary.totalHoursByStaff.push({
                staff_id: assignment.staff_id,
                staff_name: assignment.staff_name,
                total_hours: parseFloat(duration.toFixed(2))
            });
        }
    },
    updateShiftAssignmentData(state, updatedAssignment) {
        ...
        if (found && state.currentShiftData.summary) {
            if (oldStaffId === updatedAssignment.staff_id) {
                ...
                const newStaffSummary = state.currentShiftData.summary.totalHoursByStaff.find(
                    summary => summary.staff_id === updatedAssignment.staff_id
                );
                ...
                if (newStaffSummary) {
                    ...
                    state.currentShiftData.summary.totalHoursByStaff.push({
                        ...
                        staff_name: updatedAssignment.staff_name,
                        total_hours: parseFloat(newDuration.toFixed(2))
                    });
                }
            }
        }

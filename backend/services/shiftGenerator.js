...
class ShiftGeneratorService {
    ...
    initializeSession(storeId, year, month) {
        this.sessionId = `store${storeId}-${year}${month.toString().padStart(2, '0')}-${Date.now()}`;
    ...
    finalizeSession(status, result = null, error = null) {
        if (this.currentSessionLog) {
            this.currentSessionLog.endTime = new Date().toISOString();
            ...
            const logFileName = `shift-generation-${this.sessionId}.log`;
    ...
    async generateShift(storeId, year, month) {
        console.log('🚀🚀🚀 generateShift呼び出し開始:', { storeId, year, month });
        console.log('🚀🚀🚀 現在時刻:', new Date().toISOString());
        console.log('🚀🚀🚀 プロセスID:', process.pid);

        const debugLogPath = path.join(process.cwd(), 'debug-generate-shift.log');
        try {
            fs.writeFileSync(debugLogPath, `generateShift開始: ${new Date().toISOString()}\n`, { flag: 'a' });
            console.log('🚀🚀🚀 デバッグログ作成成功:', debugLogPath);
        } catch (debugError) {
            console.error('🚀🚀🚀 デバッグログ作成失敗:', debugError);
        }

        console.log('🚀🚀🚀 セッション初期化開始');
        this.initializeSession(storeId, year, month);
        console.log('🚀🚀🚀 セッション初期化完了 - sessionId:', this.sessionId);
        console.log('🚀🚀🚀 currentSessionLog存在:', !!this.currentSessionLog);

        console.log('🚀🚀🚀 ログディレクトリ:', this.logDir);
        console.log('🚀🚀🚀 ログディレクトリ存在:', fs.existsSync(this.logDir));

        try {
            if (!this.claudeApiKey) {
                throw new Error('CLAUDE_API_KEY環境変数が設定されていません。.envファイルを確認してください。');
            }
            console.log('🚀🚀🚀 Claude APIキー確認OK');

            this.logProcess('STORE_FETCH', `店舗情報取得開始`, { storeId });
            console.log('🚀🚀🚀 logProcess呼び出し完了');
            ...
            console.log('🚀🚀🚀 店舗情報取得完了:', store.name);

            console.log('🚀🚀🚀 中間ログ出力テスト開始');
            this.logProcess('TEST_PROCESS', 'テストプロセス', { test: true });

            if (this.currentSessionLog && this.logDir) {
                const testLogPath = path.join(this.logDir, `test-manual-${this.sessionId}.log`);
                try {
                    fs.writeFileSync(testLogPath, JSON.stringify(this.currentSessionLog, null, 2));
                    console.log('🚀🚀🚀 手動ログファイル作成成功:', testLogPath);
                } catch (manualError) {
                    console.error('🚀🚀🚀 手動ログファイル作成失敗:', manualError);
                }
            }
            ...
            console.log('🚀🚀🚀 スタッフ取得完了、件数:', staffs.length);
            ...
            console.log('🚀🚀🚀 プロンプト生成完了、長さ:', prompt.length);
            ...
            console.log('🚀🚀🚀 レスポンス解析完了、シフト数:', generatedShiftData.shifts.length);
            ...
            console.log('🚀🚀🚀 最終結果作成完了');
            console.log('🚀🚀🚀 finalizeSession呼び出し開始');

            this.finalizeSession('SUCCESS', finalResult);

            console.log('🚀🚀🚀 finalizeSession呼び出し完了');
            console.log('✅ generateShift完了');

            try {
                fs.writeFileSync(debugLogPath, `generateShift完了: ${new Date().toISOString()}\n`, { flag: 'a' });
            } catch (debugError) {
                console.error('🚀🚀🚀 最終デバッグログ失敗:', debugError);
            }
            ...
            console.error('❌ generateShift失敗:', error);

            try {
                fs.writeFileSync(debugLogPath, `generateShiftエラー: ${error.message} - ${new Date().toISOString()}\n`, { flag: 'a' });
            } catch (debugError) {
                console.error('🚀🚀🚀 エラーデバッグログ失敗:', debugError);
            }
